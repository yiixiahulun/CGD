...
class AtlasPlotter:
    ...
    def __init__(self, scan_results: Dict[str, Any]):
        ...
        self.alphas = self.results['alphas']

    def plot_phase_diagram(self, ax: plt.Axes):
        """绘制宇宙相图 (alpha vs. 吸引子数量)。"""
        n_attractors = self.results['n_attractors']
    ...
    def plot_lesa_deviation(self, ax: plt.Axes):
        """绘制LESA偏差图 (alpha vs. 偏差)。"""
        deviation = self.results['lesa_deviation']
    ...
    def plot_drift_trajectory(self, ax: plt.Axes):
        """绘制漂移轨迹图 (PCA投影)。"""
        pca_coords = self.results['drift_trajectory_pca']
        # pca_coords 应该是一个 (N, 2) 的数组
        sc = ax.scatter(pca_coords[:, 0], pca_coords[:, 1], c=self.alphas, cmap='viridis')
