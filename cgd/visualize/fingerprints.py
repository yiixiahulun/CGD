...
class FingerprintPlotter:
    ...
    def __init__(self, labels: List[str]):
        ...
        """
        self.K: int = len(labels)
        self.labels: List[str] = labels
        self.angles: NDArray[np.float64] = np.linspace(
            0, 2 * np.pi, self.K, endpoint=False
        ).tolist()
        # 将第一个角度放在顶部
        self.angles = np.roll(self.angles, -int(self.K / 4))

    def _setup_polar_axis(self, ax: plt.Axes, title: str) -> None:
        """配置雷达图的坐标轴。"""
    ...
    def plot_behavior_fingerprint(
        ...
        Args:
            p_vector (NDArray[np.float64]): 概率分布向量。
        ...
        if len(p_vector) != self.K:
            raise ValueError(
                f"数据维度 ({len(p_vector)}) 与标签维度 ({self.K}) 不匹配。"
            )

        # 为了闭合图形，需要将第一个点的数据追加到末尾
        values: NDArray[np.float64] = np.concatenate((p_vector, [p_vector[0]]))
        angles: NDArray[np.float64] = np.concatenate((self.angles, [self.angles[0]]))
    ...
    def plot_effect_fingerprint(
        ...
        Args:
            v_eigen (NDArray[np.float64]): 本征效应向量。
            ax (plt.Axes): 要绘制在其上的 matplotlib polar axes。
            label (str): 图例标签。
