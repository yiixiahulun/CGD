...
from ..visualize import AtlasPlotter, FingerprintPlotter, LandscapePlotter
...
def run_measurement_pipeline(
    ...
    source_map: Dict[str, GravitationalSource] = create_source_map_from_data(
        data=data,
    ...
    alpha_optimal: Optional[float] = None
    if competition_groups:
        print("\n步骤2: 正在进行一阶校准 (全局拟合 alpha)...")

        # --- 优化逻辑：只计算一次 p_map ---
        p_map: Dict[str, NDArray[np.float64]] = process_counts_to_p_dict(
            data, event_labels, group_col, event_col
        )

        comp_data_for_fitter: Dict[str, Dict[str, Any]] = {}
        for group_name, stim_names_in_group in competition_groups.items():
            ...
            full_source_list: List[str] = [stage_group_name] + stim_names_in_group
        ...
        fitter_opts: Dict[str, Any] = alpha_fitter_options or {}
...
def run_universe_analysis(
    ...
    p_observed: Optional[NDArray[np.float64]] = None,
    ...
    sources_for_analysis: List[GravitationalSource] = []
    for name in source_names_for_analysis:
        if name not in source_map:
            raise KeyError(f"错误: 在 source_map 中找不到引力源 '{name}'。")

        source: GravitationalSource = source_map[name]
        embedded_source: GravitationalSource = source.embed(new_labels=analysis_labels)
        sources_for_analysis.append(embedded_source)

        if len(source.labels) != K:
            print(
                f"  - 已将源 '{name}' 从 K={len(source.labels)} ({source.labels}) 投影到 K={K} ({tuple(analysis_labels)})。"
            )
    ...
    p_predicted: NDArray[np.float64] = equilibria[0]
    ...
    if plot and K == 3:
        try:
            ...
            tax.scatter(
                [tuple(p * 100) for p in equilibria],
                marker="*",
                color="red",
                s=150,
                label="Predicted Equilibria",
                zorder=5,
            )
            if p_observed is not None:
                tax.scatter(
                    [tuple(p_observed * 100)],
                    marker="o",
                    color="cyan",
                    s=100,
                    label="Observation",
    ...
    if p_observed is not None:
        ...
        deviation: float = distance_FR(p_predicted, p_observed)
        ...
        if deviation > 0.01:
            ...
            try:
                ...
                probe: GravitationalSource = probe_fitter.fit(
                    probe_name=f"Probe_for_{'_'.join(source_names_for_analysis)}"
                )
                print(f"发现探针: {probe}")
            except Exception as e:
                print(f"探针拟合失败: {e}")
