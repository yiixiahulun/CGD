    ...
    if isinstance(counts_data, pd.DataFrame):
        ...
        for group in all_groups:
            ...
            if subset.empty:
                ...
                warnings.warn(
                    f"在DataFrame中找不到组 '{group}' 的任何数据，将跳过该组。"
                )
                continue

            counts = (
                subset[event_col].value_counts().reindex(event_labels, fill_value=0)
            )
        ...
        for group_name, counts in counts_data.items():
            if len(counts) != K:
                raise ValueError(
                    f"组 '{group_name}' 的计数值数量 ({len(counts)}) "
                    f"与事件标签数量 ({K}) 不匹配。"
                )
            ...
            if total == 0:
                warnings.warn(
                    f"组 '{group_name}' 的总计数为0，将使用均匀分布作为替代。"
                )
                p_vec = np.full(K, 1.0 / K)
            else:
