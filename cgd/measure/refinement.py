...
class VectorInverter:
    ...
    def _objective_function_fast(self, v_eigen_candidate_flat: np.ndarray) -> float:
        ...
        candidate_source = GravitationalSource(
            ...
            v_type=cast(Literal["absolute", "substitution"], self.target_source_type),
            labels=self.labels,
        )
        ...
        return cast(float, np.sum(force_at_target**2))
    ...
    def refine(self, initial_guess_v_eigen: np.ndarray) -> GravitationalSource:
        ...
        """
        print(
            f"  - 正在为 '{self.target_source_name}' (strength≈{np.linalg.norm(initial_guess_v_eigen):.3f}) 进行精炼..."
        )
        ...
        if result.success and result.fun < 1e-9:
            print(
                f"  - 精炼成功！新 strength={np.linalg.norm(v_refined):.3f} (误差: {result.fun:.2e})"
            )
        else:
            print(
